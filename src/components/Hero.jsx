...
const ThreeCurrencyStack = lazy(() => import("./ThreeCurrencyStack"));

export default function Hero() {
  const navigate = useNavigate();

  return (
    // FIX: Added overflow-hidden to contain elements
    <section className="h-screen w-full flex items-center relative px-6 md:px-20 pt-20 pb-32 z-0 bg-transparent overflow-hidden">
        <div className="z-10 w-full md:w-[55%]">
            ...
            <div className="flex gap-6">
                <button 
                    onClick={() => navigate("/auth?type=founder")} 
                    aria-label="Launch Product"
                    className="flex items-center gap-3 bg-ethaum-green text-black px-8 py-3 rounded-full font-bold text-sm hover:scale-105 transition-transform shadow-[0_0_20px_rgba(204,255,0,0.3)]"
                >
                  Launch Product <ArrowRight size={16} />
                </button>
                <button 
                    onClick={() => navigate("/auth?type=buyer")} 
                    aria-label="Find Pilots"
