...
import { Target, Activity, ShieldCheck, Zap, Crosshair } from "lucide-react";
...
export default function MarketIntelligence() {
  ...
  const fetchIntelligence = async () => {
    const { data } = await supabase
      .from('startups')
      .select('id, name, stage, arr_range, tagline')
      .limit(12); 
  ...
  useEffect(() => {
    if (startups.length > 0) {
      const ctx = gsap.context(() => {
        // Radar Sweep
        gsap.to(".radar-sweep", {
            rotation: 360, duration: 8, repeat: -1, ease: "linear", transformOrigin: "bottom left"
        });
        // Breathing Nodes
        gsap.to(".data-node-core", {
            scale: 1.5, opacity: 0.5, duration: 1.5, repeat: -1, yoyo: true, ease: "sine.inOut", stagger: { amount: 2, from: "random" }
        });
      }, containerRef);
      return () => ctx.revert();
    }
  ...
  return (
    <StackedSection title="Market Intelligence" index={2} id="intelligence">
      <div ref={containerRef} className="w-full h-full p-6 md:p-12 flex flex-col justify-center relative">
        
        {/* LAYOUT: Two Columns directly on the grid (No outer card border) */}
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20 h-full">
            ...
            <div className="lg:w-1/3 flex flex-col justify-center relative z-20">
                ...
                <h2 className="text-5xl md:text-6xl font-light text-white mb-6 leading-[0.9] tracking-tighter">
                    Validate <br/><span className="font-bold text-white">Execution.</span>
                </h2>
                
                <p className="text-gray-400 text-sm leading-relaxed mb-8 border-l-2 border-white/10 pl-6">
                    Our AI continuously maps deep-tech startups based on verified ARR traction, IP depth, and pilot velocity.
                </p>
            ...
            <div className="lg:w-2/3 w-full flex items-center justify-center relative">
                ...
                <div className="relative w-[min(100%,500px)] aspect-square rounded-full border border-white/10 bg-black/40 backdrop-blur-sm flex items-center justify-center group overflow-hidden">
                    ...
                    {/* 4. The Sweep (Clipped by overflow-hidden parent) */}
                ...
                {hoveredNode && (
                    <div 
                        className="absolute z-50 pointer-events-none bg-black/90 backdrop-blur-xl border border-ethaum-green/30 rounded-lg p-4 w-56 flex flex-col gap-1 shadow-[0_10px_40px_rgba(0,0,0,0.8)]"
                        style={{ 
                            left: '50%', 
                            top: '50%',
                            transform: `translate(${hoveredNode.x > 50 ? '-110%' : '10%'}, ${hoveredNode.y > 50 ? '-110%' : '10%'})` 
                        }}
                    >
                        ...
                        <div className="text-[10px] text-gray-300 leading-tight">
                            {hoveredNode.tagline || "High potential deep-tech venture."}
                        </div>
                        <div className="mt-2 flex items-center gap-2 text-[9px] font-mono text-gray-500">
                             <span className="bg-white/5 px-1 rounded">{hoveredNode.stage}</span>
                             <span className="text-ethaum-green">Velocity: {hoveredNode.velocity}</span>
                        </div>
                    </div>
                )}
        ...
        {/* --- BOTTOM TICKER (Fixed Overlap) --- */}
        {/* Added z-index and explicit background to label to prevent collision visually */}
        <div className="absolute bottom-6 left-0 w-full flex items-center gap-0 px-6 md:px-12 pointer-events-none">
            
            {/* Static Label */}
            <div className="flex items-center gap-2 pr-6 bg-transparent z-10">
               <Crosshair size={12} className="text-gray-500" />
               <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest whitespace-nowrap">Market Feed</span>
            </div>

            {/* Scrolling Marquee - Masked so it fades out before hitting the label */}
                ...
                <div className="flex gap-12 animate-marquee whitespace-nowrap">
                    {startups.map((s, i) => (
                        <div key={i} className="flex items-center gap-2 text-[10px] text-gray-600 font-mono">
                            ...
                            <span>::</span> 
                            <span className="text-ethaum-green">VELOCITY_UP</span>
                        </div>
                    ))}
                </div>
